...
import { getAdsByCategory } from "../ad.service";
...
const AdDetails = () => {
  ...
  const [related, setRelated] = useState<Ad[] | null>(null);    
  const {isWishlisted,toggleWishlist}=useWish()
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      if (ad) {
        const related = await getAdsByCategory(ad.category, 3);
        setRelated(related);
      }
    };
    fetchRelated();
    ...
    <div className="bg-[#f2f4f5] min-h-screen pb-10 px-40">
      ...
      <div className="w-full relative bg-black flex justify-center items-center h-100 md:h-137.5">
        ...
        <div className="absolutetop-0 left-0 right-0 bottom-0">
          ...
          <Heart onClick={handleToggle} fill={`${isWishlisted(ad.id)?"red":""}`} color={`${isWishlisted(ad.id)?'':'white'}`} className="absolute top-4 text-white right-6" />
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="bg-white p-6 rounded-sm border border-gray-300 shadow-sm">
              <h1 className="text-2xl font-bold text-[#002f34] mb-2">
                {ad.title}
              </h1>
            ...
            {/* Overview Section */}
            <div className="bg-white p-6 rounded-sm border border-gray-300 shadow-sm">
              <h2 className="text-xl font-bold text-[#002f34] mb-4 border-b pb-2">
                Overview
              </h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                {related &&
                  related.map((item) => (
                    <div
                      key={item.id}
                      className="group border border-gray-200 rounded-lg overflow-hidden bg-white hover:shadow-md transition cursor-pointer"
                    >
                      {/* Image */}
                      ...
                      <div className="p-3 space-y-1">
                        ...
                        {/* Meta */}
                        <div className="text-xs text-gray-500 flex justify-between"></div>
                      </div>
                    </div>
                  ))}
              </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="bg-white p-6 rounded-sm border border-gray-300 shadow-sm sticky top-4">
              <h2 className="text-4xl font-bold text-[#002f34] mb-4">
                ₹ {ad.price}
              </h2>
